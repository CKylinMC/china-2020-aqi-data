...
function createRequest(method, object, callback, period) {
    const key = hex_md5(method + JSON.stringify(object));

    const data = getDataFromLocalStorage(key, period);
    if (!data) {
        var param = getParams(method, object);
        $.ajax({
...
//{"appId":"baec98a73c1bff796603cb2fa9d6d449","method":"GETMAPDATA","timestamp":1616822920069,"clienttype":"WEB","object":{"type":"DAY","timepoint":"2020-01-18"},"secret":"f19df1f9f58a11a4e42e4d4d803c2e22"}

/* run */
async function fetchData() {
    let currentDate = new Date('2020-01-01');
    let method = "GETMAPDATA";
    let url = "https://www.aqistudy.cn/apinew/aqistudyapi.php"
    while (true) {
        if (currentDate.getFullYear() != 2020) break;
        let month = currentDate.getMonth() + 1
        let timepoint = currentDate.getFullYear() + '-' + (month < 10 ? "0" + month : month) + '-' + (currentDate.getDate() < 10 ? "0" + currentDate.getDate() : currentDate.getDate())
        console.log('Quering: ', timepoint);

        let object = {
            "type": "DAY",
            "timepoint": timepoint
        }
        let param = getParams(method, object);
        let fd = new FormData();
        fd.append('h0lgYxgR3',param)
        try {
            let resp = await got.post(url, {body:fd});
            let data = resp.body;
            data = getEncryptedData(data);
            console.info('result length:'+data.length);
            try {
                fs.writeFileSync('dataset/'+timepoint + '.json', data)
                //文件写入成功。
            } catch (err) {
                console.error(timepoint + ' save failed')
            }
        } catch (ignore) {
            console.error(timepoint + ' fetch failed')
        }
